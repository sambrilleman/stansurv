...
stan_haz <- function(formula, data, dist = "weibull", df = 3,
  ...
  dist <- validate_arg(dist, "dist", ok_dists)
  ...
  t_beg <- make_t_for_stan(formula, mf, type = "beg") # beg time (for each row)
  t_end <- make_t_for_stan(formula, mf, type = "end") # end time (for each row)
  t_gap <- make_t_for_stan(formula, mf, type = "gap") # gap time (for each row)
  d <- make_d_for_stan(formula, mf) # status indicator (for each row)
  x <- make_x_for_stan(formula, mf) # fe predictor matrix
  #z <- make_z_for_stan(formula, mf) # re predictor matrix
  #g <- make_g_for_stan(formula, mf) # re group ids (for each row)

  stanfit <- stanmodels$haz
  pars <- pars_to_monitor(standata)
